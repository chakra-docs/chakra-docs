...
- `DocsPageActions` — compound page action API with `Root`, `CopyPage`, `CopyLink`, `ViewMarkdown`, `Edit`, `Menu`, `Submenu`, `Group`, `Separator`, and `Item` components. `Root` supports `sm`, `md`, and `lg` sizes plus a joined `split` variant whose recipe owns the adjoining corners and divider; menus support custom accessible trigger content, controlled or uncontrolled open state, nested disclosures, and automatic close on selection.
