...
- `DocsPageActions` — compound page action API with `Root`, `CopyPage`, `CopyLink`, `ViewMarkdown`, `Edit`, `Menu`, `Submenu`, `Group`, `Separator`, and `Item` components. `Root` supports a joined `split` variant; menus support custom accessible trigger content, controlled or uncontrolled open state, nested disclosures, and automatic close on selection.
